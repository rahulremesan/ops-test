...
# Run Flutter build web command for Widgetbook in a Docker container
echo "Running Flutter build for Widgetbook..."
docker run --rm -v ${PWD}:/app -w /app ghcr.io/cirruslabs/flutter:${FLUTTER_VERSION} \
    /bin/bash -c "flutter --version && flutter pub get && flutter build web -v -t lib/widgetbook.dart"
...
docker rmi -f ghcr.io/cirruslabs/flutter:${FLUTTER_VERSION}
